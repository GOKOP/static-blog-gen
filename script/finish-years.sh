...
for year in $(ls temp/years); do
	...
	if [ $year_n = 1 ]; then
		cat html/$1/top.html | sed 's|^.*<!-- next year !-->.*$||' > temp/top0
	else
		prev_year=$(grep "$(echo $year_n-1 | bc) " temp/year_list | awk '{print $2}')
		cat html/$1/top.html | sed 's|<!-- next year !-->|'$prev_year'|g' > temp/top0
	fi

	next_year=$(grep "$(echo $year_n+1 | bc) " temp/year_list | awk '{print $2}')

	if [ -z $next_year ]; then
		cat temp/top0 | sed 's|^.*<!-- prev year !-->.*$||' > temp/top1
	else
		cat temp/top0 | sed 's|<!-- prev year !-->|'$next_year'|g' > temp/top1
	fi

	if [ $year_n = 1 ]; then
		cat temp/top1 | sed 's|<!-- nav class !-->|nav-laft|' > temp/top2
